...
def _get_json(
    url: str, params: dict[str, Any], timeout: int = DEFAULT_TIMEOUT, retries: int = DEFAULT_RETRIES
) -> dict[str, Any]:
...
def normalize_sbdb_orbit(j: dict[str, Any]) -> dict[str, Any]:
    ...
    out = {
        ...
        "cd_epoch": cd_epoch,
        "period_d": per_d,
        "period_y": (per_d / 365.25 if per_d else None),
        "n_deg_d": n,
        "tp_jd_tdb": tp,
        "cd_tp": cd_tp,
        "H": H_val,
        ...
        "moid_au": _f(orbit.get("moid")),
        "moid_jup_au": _f(orbit.get("moid_jup")),
...
def fetch_elements_jpl1(identifier: str) -> dict[str, Any]:
    ...
    """
    j = sbdb_lookup(
        sstr=identifier, full_prec=True, soln_epoch=False, cd_epoch=True, cd_tp=True, phys_par=True
    )
    if j.get("code") == 300 and j.get("list"):
        pdes = j["list"][0].get("pdes")
        if pdes:
            j = sbdb_lookup(
                des=pdes, full_prec=True, soln_epoch=False, cd_epoch=True, cd_tp=True, phys_par=True
            )
    return normalize_sbdb_orbit(j)


# ---- Back-compat wrapper for legacy callers ----
def fetch_elements(sstr: str) -> dict[str, Any] | None:
    """
...
def cad_query(
    *,
    ...
    if dist_min:
        p["dist-min"] = dist_min
    if min_dist_min:
        p["min-dist-min"] = min_dist_min
    if min_dist_max:
        p["min-dist-max"] = min_dist_max
        ...
        out.append(
            {
                ...
                "diameter_sigma_km": to_float(get_val(row, "diameter_sigma"))
                if "diameter" in idx
                else None,
