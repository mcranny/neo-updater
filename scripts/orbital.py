...
def oe_to_rv(
    a_m: Number, e: float, i: Number, raan: Number, argp: Number, nu: Number, mu: float = MU_SUN
    ...
    v_pf = np.array([-math.sqrt(mu / p) * snu, math.sqrt(mu / p) * (e + cnu), 0.0], dtype=float)
...
def stumpff_C(z: Number) -> float:
    ...
    s = math.sqrt(-z)
    if s > 700.0:
        raise ValueError("Stumpff C overflow: transfer iterate left the supported domain")
...
def stumpff_S(z: Number) -> float:
    ...
    s = math.sqrt(-z)
    if s > 700.0:
        raise ValueError("Stumpff S overflow: transfer iterate left the supported domain")
...
def lambert_universal(
    ...
    def y(z: float) -> float:
        ...
        return r1n + r2n + A * ((z * S - 1.0) / math.sqrt(Ceff))
    ...
    # Locate the first finite single-revolution root.  Invalid y(z) regions
    # are skipped instead of being treated as +infinity, which can produce a
    # false bisection bracket and a visually plausible but incorrect arc.
    z_limit = 4.0 * math.pi**2 - 1e-6
    previous: tuple[float, float] | None = None
    bracket: tuple[float, float] | None = None
    for candidate in np.linspace(-z_limit, z_limit, 1200):
        try:
            value = tof(float(candidate)) - tof_s
        ...
        if previous is not None and previous[1] * value < 0.0:
            bracket = (previous[0], float(candidate))
            break
        previous = (float(candidate), value)
    if bracket is None:
        raise RuntimeError("Lambert: no single-revolution time-of-flight root")
    ...
    for _ in range(max_iter):
        ...
        if f_lo * f_mid <= 0.0:
            ...
            z_lo = z
    if not converged:
        # A nanosecond absolute tolerance is unreasonable at multi-month
        # scales; the interval criterion provides a stable secondary stop.
        converged = abs(z_hi - z_lo) < 1e-12
        z = 0.5 * (z_lo + z_hi)
    if not converged:
        raise RuntimeError("Lambert: time-of-flight root did not converge")
    ...
    v1 = (r2 - f * r1) / g
    v2 = (gdot * r2 - r1) / g
    propagated_r2, _ = kepler_universal_propagate(r1, v1, tof_s, mu)
    residual_m = float(np.linalg.norm(propagated_r2 - r2))
    tolerance_m = max(10_000.0, 1e-7 * r2n)
    if not math.isfinite(residual_m) or residual_m > tolerance_m:
        raise RuntimeError(
            f"Lambert endpoint residual {residual_m:.3e} m exceeds {tolerance_m:.3e} m"
        )
    return v1, v2
...
def kepler_universal_propagate(
    r0, v0, dt: Number, mu: float = MU_SUN, itmax: int = 80, tol: float = 1e-6
):
    """
    ...
    if abs(alpha) > 1e-12:
        ...
        # Near-parabolic initial guess from the semilatus rectum.
        h = float(np.linalg.norm(np.cross(r0, v0)))
        p = h * h / mu
        if p <= 0.0:
            raise ValueError("kepler_universal_propagate: degenerate angular momentum")
    ...
    for _ in range(itmax):
        ...
        Sz = stumpff_S(z)
        F = (
            (r0_dot_v0 / sqrt_mu) * chi * chi * Cz
            + (1.0 - alpha * r0n) * chi**3 * Sz
            + r0n * chi
            - sqrt_mu * dt
        )
        dF = (
            (r0_dot_v0 / sqrt_mu) * chi * (1.0 - z * Sz)
            + (1.0 - alpha * r0n) * chi * chi * Cz
            + r0n
        )
        if abs(F) < tol * sqrt_mu:
            ...
            break
        if abs(dF) < 1e-16:
            raise RuntimeError("Kepler propagation derivative vanished")
        chi -= F / dF
    if not converged:
        raise RuntimeError("Kepler universal-variable solver did not converge")
