...
```mermaid
flowchart LR
    CAD[JPL CAD<br/>close approaches] --> ING[ingestion]
    SBDB[JPL SBDB<br/>orbital elements] --> ING
    ING --> SQL[(SQLite)]
    SQL --> NATIVE[native catalog / SQL]
    SQL --> FLASK[Flask fallback / API]
    SQL --> JSON[JSON export] --> VIEWER[3D viewer]
```
